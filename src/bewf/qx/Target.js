...
/**
 * This is the main constructor for all objects that need to be connected to jsw.event.Event objects.
 *
 ...
 */
jsw.qx.Class.define( "jsw.qx.Target", {
  ...
  statics : {
    ...
    _addToPendingEventsQueue : function( eventContext, queue ) {
      if (jsw.qx.Target._autoFlushTimeout == null) {
    	jsw.qx.Target._initAutoFlush();
      }
      
      if( queue == jsw.qx.Target.REMOVE_QUEUE){
          jsw.qx.Target._pendingEventsRemoveQueue.push( eventContext);
      } else if( queue == jsw.qx.Target.SET_QUEUE){
          jsw.qx.Target._pendingEventsSetQueue.push( eventContext);
      } else if( queue == jsw.qx.Target.CONNECT_QUEUE){
          jsw.qx.Target._pendingEventsConnectQueue.push( eventContext);
      } else if( queue == jsw.qx.Target.DISCONNET_QUEUE){
          jsw.qx.Target._pendingEventsDisconnectQueue.push( eventContext);
      } else if( queue == jsw.qx.Target.CREATE_QUEUE){
          jsw.qx.Target._pendingEventsCreateQueue.push( eventContext);
      } else {
    	  // unknown queue 
      }
    },
    ...
    _initAutoFlush : function(  ) {
      if( jsw.qx.Target._autoFlushTimeout == null ) {
    	  jsw.qx.Target._autoFlushTimeout
              = window.setTimeout( jsw.qx.Target._autoFlushHelper, 0 );
      }
    },
    
    _autoFlushHelper : function() {
      try {
    	jsw.qx.Target._autoFlushTimeout = null;
        if( !jsw.qx.Object.inGlobalDispose() ) {
        	jsw.qx.Target._flushPendingEventsQueues();
        }
      }catch( ex ) {
    ...
    _flushPendingEventsQueues : function() {
      if( jsw.qx.Target._autoFlushTimeout != null ) {
    	  jsw.qx.Target._removeAutoFlush();
      }
      if( jsw.qx.Target._inFlushPendingEventsQueues ) {
        return;
      }
      jsw.qx.Target._inFlushPendingEventsQueues = true;
      jsw.qx.Target._flushPendingEventsQueue(jsw.qx.Target.CONNECT_QUEUE);
      jsw.qx.Target._flushPendingEventsQueue(jsw.qx.Target.REMOVE_QUEUE);
      jsw.qx.Target._flushPendingEventsQueue(jsw.qx.Target.CREATE_QUEUE);
      jsw.qx.Target._flushPendingEventsQueue(jsw.qx.Target.SET_QUEUE);
      jsw.qx.Target._flushPendingEventsQueue(jsw.qx.Target.DISCONNET_QUEUE);
      delete jsw.qx.Target._inFlushPendingEventsQueues;
    },
  
    _removeAutoFlush : function() {
      if( jsw.qx.Target._autoFlushTimeout != null ) {
        window.clearTimeout( jsw.qx.Target._autoFlushTimeout );
        jsw.qx.Target._autoFlushTimeout = null;
      }
    },
    ...
    _flushPendingEventsQueue : function( queue ) {
      ...
      if( queue == jsw.qx.Target.REMOVE_QUEUE){
    	  vQueue = jsw.qx.Target._pendingEventsRemoveQueue;
      } else if( queue == jsw.qx.Target.SET_QUEUE){
    	  vQueue = jsw.qx.Target._pendingEventsSetQueue;
      } else if( queue == jsw.qx.Target.CONNECT_QUEUE){
    	  vQueue = jsw.qx.Target._pendingEventsConnectQueue;
      } else if( queue == jsw.qx.Target.DISCONNET_QUEUE){
    	  vQueue = jsw.qx.Target._pendingEventsDisconnectQueue;
      } else if( queue == jsw.qx.Target.CREATE_QUEUE){
    	  vQueue = jsw.qx.Target._pendingEventsCreateQueue;
      } else {
    	  // unknown queue 
      }
      ...
      while ((vLength = vQueue.length) > 0) {
        for (var i=0; i<vLength; i++) {
          ...
          jsw.qx.Target._callFunction(vContextEvent.func, vContextEvent.obj, vContextEvent.evt);
        }
        vQueue.splice(0, vLength);
      }
      
      if( queue == jsw.qx.Target.REMOVE_QUEUE){
    	  jsw.qx.Target._pendingEventsRemoveQueue = [];
      } else if( queue == jsw.qx.Target.SET_QUEUE){
    	  jsw.qx.Target._pendingEventsSetQueue = [];
      } else if( queue == jsw.qx.Target.CONNECT_QUEUE){
    	  jsw.qx.Target._pendingEventsConnectQueue = [];
      } else if( queue == jsw.qx.Target.DISCONNET_QUEUE){
    	  jsw.qx.Target._pendingEventsDisconnectQueue = [];
      } else if( queue == jsw.qx.Target.CREATE_QUEUE){
    	  jsw.qx.Target._pendingEventsCreateQueue = [];
      } else {
    	  // unknown queue 
      }
    },
  ...
  members : {
    ...
    addEventListener : function( type, func, obj ) {
      ...
      var key = "event" + jsw.qx.Object.toHashCode( func ) +
        ( obj ? "$" + jsw.qx.Object.toHashCode( obj ) : "" );
    ...
    removeEventListener : function( type, func, obj ) {
      ...
      if( typeof func !== "function" ) {
        throw new Error( "jsw.qx.Target: removeEventListener(" + type + "): '" + func
                         + "' is not a function!" );
      }
    ...
    hasEventListeners : function( type ) {
      return this.__listeners &&
        ...
        !jsw.util.Objects.isEmpty( this.__listeners[ type ] );
    },
    ...
    createDispatchEvent : function( type ) {
      if( this.hasEventListeners( type ) ) {
        this.dispatchEvent( new jsw.event.Event( type ), true );
      }
    },
    ...
    createDispatchDataEvent : function( type, data ) {
      if( this.hasEventListeners( type ) ) {
        this.dispatchEvent( new jsw.event.DataEvent( type, data ), true );
      }
    },
    ...
    createDispatchChangeEvent : function( type, value, old ) {
      if( this.hasEventListeners( type ) ) {
        this.dispatchEvent( new jsw.event.ChangeEvent( type, value, old ), true );
      }
    },
    ...
    _callFunction : function(func, obj, evt, async, queue ){
      if( !async ){
        ...
        jsw.qx.Target._addToPendingEventsQueue(eventContext, queue);
      }
  	  return;
    }, 
    ...
    _allowDispatch : function( event ) {
      var result = true;
      if( this.getEnabled && event instanceof jsw.event.DomEvent ) {

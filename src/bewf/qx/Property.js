 ...
 * through the methods provided by {@link jsw.qx.Class}.
 *
 ...
 * *Normal properties*
 *
 * The <code>properties</code> key in the class definition map of {@link jsw.qx.Class#define}
 ...
 *   - a class name e.g. <code>jsw.widgets.base.Button</code>
 ...
 *   type is {@link jsw.event.ChangeEvent}.
 * - themeable {Boolean}
 ...
 *   Whether this property can be set using themes.
 */
jsw.qx.Class.define( "jsw.qx.Property", {
  ...
  statics : {
    ...
    __checks : {
      ...
      "Map"       : 'value !== null && typeof value === "object" && !(value instanceof Array) && !(value instanceof jsw.qx.Object)',
      ...
      "Color"     : 'typeof value === "string" && jsw.util.Colors.isValid(value)',
      "Border"    : 'value !== null',
      "Font"      : 'value !== null',
    ...
    /** Contains names of inheritable properties, filled by {@link jsw.qx.Class.define} */
    $$inheritable : {},

    /**
     * Refreshes widget whose parent has changed (including the children)
     *
     * @param widget {jsw.widgets.base.Widget} the widget
     */
    ...
    attachMethods : function( clazz, name, config ) {
      ...
      if( name.charAt(0) === "_" ) {
        if( name.charAt(1) === "_" ) {
          prefix = "__";
          postfix = jsw.util.Strings.toFirstUp(name.substring(2));
        } else {
          prefix = "_";
          postfix = jsw.util.Strings.toFirstUp(name.substring(1));
        }
      } else {
        prefix = "";
        postfix = jsw.util.Strings.toFirstUp(name);
      }
    ...
    __attachGroupMethods : function( clazz, config, prefix, postfix ) {
      ...
      if( config.mode == "shorthand" ) {
        var shorthand = "a=jsw.util.Arrays.fromShortHand(jsw.util.Arrays.fromArguments(a));";
      ...
      for( var i = 0, a = config.group, l = a.length; i < l; i++ ) {
        if( jsw.util.Variant.isSet( "qx.debug", "on" ) ) {
    ...
    __attachPropertyMethods : function( clazz, config, prefix, postfix ) {
      ...
      if( config.dispose === undefined && typeof config.check === "string" ) {
        config.dispose = this.__dispose[ config.check ] || jsw.qx.Class.isDefined( config.check );
      }
      ...
      members[method.get[name]] = function() {
        return jsw.qx.Property.executeOptimizedGetter( this, clazz, name, "get" );
      };

      method.set[name] = prefix + "set" + postfix;
      members[method.set[name]] = function( value ) {
        return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "set", arguments );
      };

      method.reset[name] = prefix + "reset" + postfix;
      members[method.reset[name]] = function() {
        return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "reset" );
      };
      ...
      if( config.inheritable || config.apply || config.event || config.deferredInit ) {
        ...
        members[method.init[name]] = function(value) {
          return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "init", arguments );
        };
      }
      ...
      if( config.inheritable ) {
        ...
        members[method.refresh[name]] = function( value ) {
          return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "refresh", arguments );
        };
      }
      ...
      if( config.themeable ) {
        ...
        members[method.style[name]] = function( value ) {
          return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "style", arguments );
        };

        method.unstyle[name] = prefix + "unstyle" + postfix;
        members[method.unstyle[name]] = function() {
          return jsw.qx.Property.executeOptimizedSetter( this, clazz, name, "unstyle" );
        };
      }
     ...
     *
     * @param obj {jsw.qx.Object} Any qooxdoo object
    ...
    __unwrapFunctionFromCode : function(instance, members, name, variant, code, args ) {
      ...
      } else if( jsw.util.Variant.isSet( "qx.debug", "on" ) ) {
    ...
    executeOptimizedSetter : function( instance, clazz, name, variant, args ) {
      ...
      if( jsw.util.Variant.isSet( "qx.debug", "on" ) ) {
        code.push( 'var prop=jsw.qx.Property;' );
        if( variant === "init" ) {
        ...
        if( jsw.util.Variant.isSet( "qx.debug", "on" ) && variant === "set" ) {
          code.push( 'if(value===undefined)prop.error(this,2,"' + name + '","' + variant + '",value);' );
        }
      }
      ...
      // TODO [rh] unused: changed as in patch to http://bugzilla.qooxdoo.org/show_bug.cgi?id=599
      if( incomingValue && jsw.util.Variant.isSet( "qx.debug", "on" ) ) {
        // Null check
        if( !config.nullable ) {
        ...
        if( config.check !== undefined ) {
          ...
          if( this.__checks[config.check] !== undefined ) {
          ...
          else if( jsw.qx.Class.isDefined( config.check ) ) {
            ...
            config.checkMap = jsw.util.Objects.fromArray(config.check);

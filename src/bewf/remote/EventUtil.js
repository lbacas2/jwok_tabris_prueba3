...
jsw.remote.EventUtil = {
  ...
  _altKey : false,
  _metaKey : false,
  _button : '', //jsw.event.MouseEvent.C_BUTTON_NONE,

  eventTimestamp : function() {
    var init = jsw.runtime.System.getInstance();
  ...
  addButtonToProperties : function( properties, event ) {
    ...
    switch( button ) {
      case jsw.event.MouseEvent.C_BUTTON_LEFT:
        properties.button = 1;
        break;
      case jsw.event.MouseEvent.C_BUTTON_MIDDLE:
        properties.button = 2;
        break;
      case jsw.event.MouseEvent.C_BUTTON_RIGHT:
        properties.button = 3;
        break;
    }
  },
  */
  ...
  addModifierToProperties : function( properties, event ) {
    var isMac = jsw.client.Client.getPlatform() === "mac";
  ...
  */
  ...
  focusGained : function( event ) {
    if( !self.getSuspended() ) {
      var remoteObject = jsw.remote.Connection.getInstance().getRemoteObject( event.getTarget() );
  ...
  mouseDoubleClick : function( event ) {
    if( !self.getSuspended() && self._isRelevantMouseEvent( this, event ) ) {
      ...
      if( self._isDoubleClick( this, event ) ) {
        ...
        jsw.client.Timer.once( self._clearLastMouseDown, this, self.DOUBLE_CLICK_TIME );
      }
    }
  },
  ...
  */
  ...
  _isDoubleClick : function( widget, event ) {
    ...
    return    lastMouseDown.mouseUpCount === 1
           ...
           && lastMouseDown.button === jsw.event.MouseEvent.C_BUTTON_LEFT
           && lastMouseDown.button === event.getButton()
  ...
  _notifyMouseListeners : function( widget, event, eventType ) {
    ...
    self.addModifierToProperties( properties );
    jsw.remote.Connection.getInstance().getRemoteObject( widget ).notify( eventType, properties );
  },
  */
  ...
  helpRequested : function( event ) {
    if( event.getKeyIdentifier() === "F1" ) {
      ...
      if( id != null ) {
        var remoteObject = jsw.remote.Connection.getInstance().getRemoteObject( widget );
  ...
  menuDetectedByKey : function( event ) {
    if( event.getKeyIdentifier() === "Apps" ) {
      ...
      event.stopPropagation();
      event.preventDefault();
      var x = jsw.event.MouseEvent.getPageX();
      var y = jsw.event.MouseEvent.getPageY();
      self.sendMenuDetected( event.getTarget(), x, y );
    }
  },

  menuDetectedByMouse : function( event ) {
    if( event.getButton() === jsw.event.MouseEvent.C_BUTTON_RIGHT ) {
      // stop further handling and default handling by the browser
      event.stopPropagation();
      event.preventDefault();

...
jsw.runtime.MobileWebkitSupport = {
  ...
  _draggableTypes : {
    "jsw.widgets.Shell" : null,
    "jsw.widgets.Sash"  : null,
    "jsw.widgets.Scale" : [ "scale-thumb" ],
    "jsw.widgets.Slider" : [ "slider-thumb" ],
    "jsw.widgets.base.ScrollBar" : null,
    "jsw.widgets.ScrolledComposite" : [ "scrollbar-thumb" ],
    "jsw.widgets.base.BasicButton" : [ "scrollbar-thumb" ],
    "jsw.widgets.base.Parent" : [ "coolitem-handle" ],
    "jsw.widgets.List" : [ "scrollbar-thumb" ],
    "jsw.widgets.Grid" : [ "tree-column", "label", "image", "scrollbar-thumb" ]
  },
  ...
  init : function() {
    if( jsw.client.Client.supportsTouch() ) {
      ...
      this.setTouchScrolling( !jsw.client.Client.isAndroidBrowser() );
    }
  },
  ...
  _configureToolTip : function() {
    var toolTip = jsw.widgets.base.WidgetToolTip.getInstance();
    toolTip.setMousePointerOffsetX( -35 );
    toolTip.setMousePointerOffsetY( -100 );
    var manager = jsw.widgets.util.ToolTipManager.getInstance();
  ...
  _hideTabHighlight : function() {
    jsw.html.StyleSheet.createElement( " * { -webkit-tap-highlight-color: rgba(0,0,0,0); }" );
  },

  _bindListeners : function() {
     this.__onTouchEvent = jsw.util.Functions.bind( this._onTouchEvent, this );
     this.__onGestureEvent = jsw.util.Functions.bind( this._onGestureEvent, this );
     this.__onOrientationEvent = jsw.util.Functions.bind( this._onOrientationEvent, this );
  },
  ...
  _registerFilter : function() {
    jsw.event.EventHandler.setMouseDomEventFilter( this._filterMouseEvents, this );
  },
  ...
  _onTouchEvent : function( domEvent ) {
    try {
      if( !jsw.remote.EventUtil.getSuspended() ) {
  ...
  _handleTouchStart : function( domEvent ) {
    ...
    var widgetTarget = jsw.event.EventHandlerUtil.getOriginalTargetObject( target );
  ...
  _initVirtualScroll : function( widget ) {
    var scrollable;
    if( widget instanceof jsw.widgets.base.GridRow ) {
  ...
  _isFocusable : function( widgetTarget ) {
    return widgetTarget instanceof jsw.widgets.base.BasicText;
  },
  ...
  _isGridRow : function( widgetTarget ) {
    return widgetTarget instanceof jsw.widgets.base.GridRow;
  },

  _isSelectableWidget : function( widgetTarget ) {
    var result = false;
    if(    widgetTarget instanceof jsw.widgets.ListItem
        || widgetTarget instanceof jsw.widgets.base.GridRow )
    {
      result = true;
    }
    return result;
  ...
  _findScrollable : function( widget ) {
    ...
    do {
      if( currentWidget instanceof jsw.widgets.base.Scrollable ) {
        result = currentWidget;
      } else if( currentWidget instanceof jsw.widgets.base.ClientDocument ) {
        currentWidget = null;
      } else {
  ...
  _isDoubleClick : function( domEvent ) {
    ...
    if( this._lastMouseClickTarget === target ) {
      ...
      result = diff < jsw.remote.EventUtil.DOUBLE_CLICK_TIME;
    }
    return result;
  },
  ...
  _getDummyTarget : function() {
    return jsw.widgets.base.ClientDocument.getInstance()._getTargetNode();
  },
  ...
  _fireMouseEvent : function( type, target, originalEvent, coordiantes ) {
    ...
    event.initMouseEvent( type,
                          ...
                          false, //shiftKey
                          false, //metaKey
                          jsw.event.MouseEvent.buttons.left,
                          null );
  ...
  _postMouseEvent : function( type ) {
    if( type === "mouseup" ) {
      jsw.widgets.util.ToolTipManager.getInstance().setCurrentToolTipTarget( null );
    }
  }

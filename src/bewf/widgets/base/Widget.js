 ...
 *
 * @appearance widget
 * @state selected Set by {@link jsw.widgets.util.SelectionManager#renderItemSelectionState}
 * @state anchor Set by {@link jsw.widgets.util.SelectionManager#renderItemAnchorState}
 * @state lead Set by {@link jsw.widgets.util.SelectionManager#renderItemLeadState}
 *
 * @state disabled Set by {@link jsw.qx.Object#enabled}
 * @state focused Set by {@link #focused}
 */
jsw.qx.Class.define( "jsw.widgets.base.Widget", {
  ...
  events: {
    "beforeAppear"    : "jsw.event.Event",
    "appear"          : "jsw.event.Event",
    "beforeDisappear" : "jsw.event.Event",
    "disappear"       : "jsw.event.Event",
    "insertDom"       : "jsw.event.Event",
    "create"          : "jsw.event.Event",
    "execute"         : "jsw.event.Event",
    "flush"           : "jsw.event.DataEvent",

    /** (Fired by {@link jsw.event.EventHandler}) */
    "mouseover"       : "jsw.event.MouseEvent",
    "mousemove"       : "jsw.event.MouseEvent",
    "mouseout"        : "jsw.event.MouseEvent",
    "mousedown"       : "jsw.event.MouseEvent",
    "mouseup"         : "jsw.event.MouseEvent",
    "mousewheel"      : "jsw.event.MouseEvent",
    "click"           : "jsw.event.MouseEvent",
    "dblclick"        : "jsw.event.MouseEvent",
    "contextmenu"     : "jsw.event.MouseEvent",
    "elementOver"     : "jsw.event.MouseEvent",
    "elementOut"      : "jsw.event.MouseEvent",
    "keydown"         : "jsw.event.KeyEvent",
    "keypress"        : "jsw.event.KeyEvent",
    "keyup"           : "jsw.event.KeyEvent",

    /** (Fired by {@link jsw.widgets.base.Parent}) */
    "focusout"        : "jsw.event.FocusEvent",
    "focusin"         : "jsw.event.FocusEvent",
    "blur"            : "jsw.event.FocusEvent",
    "focus"           : "jsw.event.FocusEvent",

    /** (Fired by {@link jsw.event.DragAndDropHandler}) */
    "dragdrop"        : "jsw.event.DragEvent",
    "dragout"         : "jsw.event.DragEvent",
    "dragover"        : "jsw.event.DragEvent",
    "dragmove"        : "jsw.event.DragEvent",
    "dragstart"       : "jsw.event.DragEvent",
    "dragend"         : "jsw.event.DragEvent"
  },
  ...
  statics : {
    ...
    _initAutoFlush : function( phase ) {
      if( jsw.widgets.base.Widget._autoFlushTimeout == null ) {
        // RAP: Fix for bug 303162
        if( !jsw.widgets.base.Widget._inFlushGlobalQueues || phase < jsw.widgets.base.Widget._flushGlobalQueuesPhase ) {
          jsw.widgets.base.Widget._autoFlushTimeout = window.setTimeout( jsw.widgets.base.Widget._autoFlushHelper, 0 );
        }
      }
    },
    ...
    _removeAutoFlush : function() {
      if( jsw.widgets.base.Widget._autoFlushTimeout != null ) {
        window.clearTimeout( jsw.widgets.base.Widget._autoFlushTimeout );
        jsw.widgets.base.Widget._autoFlushTimeout = null;
      }
    },
    ...
    _autoFlushHelper : function() {
      try {
        jsw.widgets.base.Widget._autoFlushTimeout = null;
        if( !jsw.qx.Object.inGlobalDispose() ) {
    ...
    flushGlobalQueues : function() {
      if( jsw.widgets.base.Widget._autoFlushTimeout != null ) {
        jsw.widgets.base.Widget._removeAutoFlush();
      }
      if( jsw.widgets.base.Widget._inFlushGlobalQueues ) {
        return;
      }
      if( !jsw.runtime.System.getInstance().getUiReady() ) {
        return;
      }
      jsw.widgets.base.Widget._inFlushGlobalQueues = true;
      jsw.widgets.base.Widget.flushGlobalWidgetQueue();
      jsw.widgets.base.Widget.flushGlobalStateQueue();
      jsw.widgets.base.Widget.flushGlobalElementQueue();
      jsw.widgets.base.Widget.flushGlobalJobQueue();
      jsw.widgets.base.Widget.flushGlobalLayoutQueue();
      jsw.widgets.base.Widget.flushGlobalDisplayQueue();
      jsw.widgets.base.Widget.flushGlobalDisposeQueue();
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_IDLE;
      delete jsw.widgets.base.Widget._inFlushGlobalQueues;
    },

    addToGlobalWidgetQueue : function(vWidget) {
      if (!vWidget._isInGlobalWidgetQueue && vWidget._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_WIDGET );
        }
        jsw.widgets.base.Widget._globalWidgetQueue.push(vWidget);
    ...
    removeFromGlobalWidgetQueue : function(vWidget) {
      if (vWidget._isInGlobalWidgetQueue) {
        jsw.util.Arrays.remove(jsw.widgets.base.Widget._globalWidgetQueue, vWidget);
        delete vWidget._isInGlobalWidgetQueue;
      }
    },

    flushGlobalWidgetQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_WIDGET;
      var vQueue = jsw.widgets.base.Widget._globalWidgetQueue, vLength, vWidget;
      ...
      jsw.widgets.base.Widget._globalWidgetQueue = [];
    },

    addToGlobalElementQueue : function(vWidget) {
      if (!vWidget._isInGlobalElementQueue && vWidget._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_ELEMENT );
        }
        jsw.widgets.base.Widget._globalElementQueue.push(vWidget);
    ...
    removeFromGlobalElementQueue : function(vWidget) {
      if (vWidget._isInGlobalElementQueue) {
        jsw.util.Arrays.remove(jsw.widgets.base.Widget._globalElementQueue, vWidget);
        delete vWidget._isInGlobalElementQueue;
      }
    },

    flushGlobalElementQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_ELEMENT;
      var vQueue = jsw.widgets.base.Widget._globalElementQueue, vLength, vWidget;
      while ((vLength = vQueue.length) > 0) {
        for (var i=0; i<vLength; i++) {
          vWidget = vQueue[i];
      ...
      jsw.widgets.base.Widget._globalElementQueue = [];
    },

    addToGlobalStateQueue : function(vWidget) {
      if (!vWidget._isInGlobalStateQueue && vWidget._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_STATE );
        }
        jsw.widgets.base.Widget._globalStateQueue.push(vWidget);
    ...
    removeFromGlobalStateQueue : function(vWidget) {
      if (vWidget._isInGlobalStateQueue) {
        jsw.util.Arrays.remove(jsw.widgets.base.Widget._globalStateQueue, vWidget);
        delete vWidget._isInGlobalStateQueue;
      }
    },

    flushGlobalStateQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_STATE;
      ...
      while(Widget._globalStateQueue.length > 0) {
        var queue = jsw.util.Arrays.copy(Widget._globalStateQueue);
    ...
    addToGlobalJobQueue : function(vWidget) {
      if (!vWidget._isInGlobalJobQueue && vWidget._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_JOB );
        }
        jsw.widgets.base.Widget._globalJobQueue.push(vWidget);
    ...
    removeFromGlobalJobQueue : function(vWidget) {
      if (vWidget._isInGlobalJobQueue) {
        jsw.util.Arrays.remove(jsw.widgets.base.Widget._globalJobQueue, vWidget);
        delete vWidget._isInGlobalJobQueue;
      }
    },

    flushGlobalJobQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase
        = jsw.widgets.base.Widget._FLUSH_PHASE_JOB;
      var vQueue = jsw.widgets.base.Widget._globalJobQueue, vLength, vWidget;
      while ((vLength = vQueue.length) > 0) {
        for (var i=0; i<vLength; i++) {
          vWidget = vQueue[i];
      ...
      jsw.widgets.base.Widget._globalJobQueue = [];
    },

    addToGlobalLayoutQueue : function(vParent) {
      if (!vParent._isInGlobalLayoutQueue && vParent._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_LAYOUT );
        }
        jsw.widgets.base.Widget._globalLayoutQueue.push(vParent);
    ...
    removeFromGlobalLayoutQueue : function(vParent) {
      if (vParent._isInGlobalLayoutQueue) {
        jsw.util.Arrays.remove(jsw.widgets.base.Widget._globalLayoutQueue, vParent);
        delete vParent._isInGlobalLayoutQueue;
      }
    },

    flushGlobalLayoutQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_LAYOUT;
      var vQueue = jsw.widgets.base.Widget._globalLayoutQueue, vLength, vParent;
      while ((vLength = vQueue.length) > 0) {
        for (var i=0; i<vLength; i++) {
          vParent = vQueue[i];
      ...
      jsw.widgets.base.Widget._globalLayoutQueue = [];
    },

    addToGlobalDisplayQueue : function(vWidget) {
      if (!vWidget._isInGlobalDisplayQueue && vWidget._isDisplayable) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_DISPLAY );
        }
        var vParent = vWidget.getParent();
        if (vParent.isSeeable()) {
          var vKey = vParent.toHashCode();
          if (jsw.widgets.base.Widget._lazyGlobalDisplayQueues[vKey]) {
            jsw.widgets.base.Widget._lazyGlobalDisplayQueues[vKey].push(vWidget);
          } else {
            jsw.widgets.base.Widget._lazyGlobalDisplayQueues[vKey] = [ vWidget ];
          }
        } else {
          jsw.widgets.base.Widget._fastGlobalDisplayQueue.push(vWidget);
        }
    ...
    flushGlobalDisplayQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase = jsw.widgets.base.Widget._FLUSH_PHASE_DISPLAY;
      var vKey, vLazyQueue, vWidget, vFragment;
      var vFastQueue = jsw.widgets.base.Widget._fastGlobalDisplayQueue;
      var vLazyQueues = jsw.widgets.base.Widget._lazyGlobalDisplayQueues;
      // Work on fast queue
      ...
      // because through the recursion from jsw.widgets.base.Parent
      ...
      if (jsw.util.Objects.isEmpty(vLazyQueues)) {
        jsw.widgets.base.Widget._lazyGlobalDisplayQueues = {};
      }
      ...
      jsw.widgets.base.Widget._fastGlobalDisplayQueue = [];
    },

    addToGlobalDisposeQueue : function(vWidget) {
      if (!vWidget._isInGlobalDisposeQueue && !vWidget.isDisposed()) {
        if (jsw.widgets.base.Widget._autoFlushTimeout == null) {
          jsw.widgets.base.Widget._initAutoFlush( jsw.widgets.base.Widget._FLUSH_PHASE_DISPOSE );
        }
        jsw.widgets.base.Widget._globalDisposeQueue.push(vWidget);
    ...
    flushGlobalDisposeQueue : function() {
      jsw.widgets.base.Widget._flushGlobalQueuesPhase
        = jsw.widgets.base.Widget._FLUSH_PHASE_DISPOSE;
      var vQueue = jsw.widgets.base.Widget._globalDisposeQueue, vLength, vWidget;
      while ((vLength = vQueue.length) > 0) {
        for (var i=0; i<vLength; i++) {
          vWidget = vQueue[i];
          vWidget.dispose();
      ...
      jsw.widgets.base.Widget._globalDisposeQueue = [];
    },
    ...
    getActiveSiblingHelper : function(vObject, vParent, vCalc, vIgnoreClasses, vMode) {
      ...
      while (vInstance && (!vInstance.getEnabled() || jsw.widgets.base.Widget.getActiveSiblingHelperIgnore(vIgnoreClasses, vInstance))) {
        vPosition += vCalc;
    ...
    __initLayoutProperties : function(statics) {
      ...
      for (var i=0, l=a.length, p, b, t; i<l; i++) {
        p = a[i];
        b = "_computed" + jsw.util.Strings.toFirstUp(p);
        t = b + "Type";
    ...
    disableScrolling : function( widget ) {
      ...
      if( el ) {
        jsw.html.Scroll.disableScrolling(el);
      } else {
    ...
    enableScrolling : function( widget ) {
      var el = widget._getTargetNode();
      if( el ) {
        jsw.html.Scroll.enableScrolling( el );
      } else {
    ...
    _blockScrollingOnAppear : function() {
      var func = jsw.widgets.base.Widget._blockScrollingOnAppear;
      this.removeEventListener( "appear", func, this );
      jsw.widgets.base.Widget.disableScrolling( this );
    }
  ...
  properties : {
     ...
     *  Used by some layout handlers (jsw.widgets.base.BoxLayout, ...).
     */
    allowStretchX : {
      check : "Boolean",
      init : true
  ...
  */
  ...
  members : {
    ...
    getChildren : jsw.util.Functions.returnNull,
    getChildrenLength : jsw.util.Functions.returnZero,
    hasChildren : jsw.util.Functions.returnFalse,
    isEmpty : jsw.util.Functions.returnTrue,
    indexOf : jsw.util.Functions.returnNegativeIndex,
    contains : jsw.util.Functions.returnFalse,
    getVisibleChildren : jsw.util.Functions.returnNull,
    getVisibleChildrenLength : jsw.util.Functions.returnZero,
    hasVisibleChildren : jsw.util.Functions.returnFalse,
    isVisibleEmpty : jsw.util.Functions.returnTrue,
    ...
    _applyParent : function(value, old) {
      if (old) {
        ...
        jsw.util.Arrays.removeAt(old.getChildren(), vOldIndex);
...
// TODO: Remove
      ...
      if (value) {
        ...
        if (typeof this._insertIndex == "number") {
          jsw.util.Arrays.insertAt(value.getChildren(), this, this._insertIndex);
...
//          vParent.getLayoutImpl().updateChildrenOnAddChild( this, index );
        ...
        if( !this._isCreated ) {
          jsw.widgets.base.Widget.addToGlobalElementQueue(this);
        }
        jsw.widgets.base.Widget.addToGlobalStateQueue( this );
        if(!jsw.util.Objects.isEmpty(this._jobQueue ) ) {
          jsw.widgets.base.Widget.addToGlobalJobQueue( this );
        }
        if( !jsw.util.Objects.isEmpty( this._childrenQueue ) ) {
          jsw.widgets.base.Widget.addToGlobalLayoutQueue( this );
        }
        // displayable end
      } else {
        jsw.widgets.base.Widget.removeFromGlobalElementQueue( this );
        jsw.widgets.base.Widget.removeFromGlobalStateQueue( this );
        jsw.widgets.base.Widget.removeFromGlobalJobQueue( this );
        jsw.widgets.base.Widget.removeFromGlobalLayoutQueue( this );
...
//            vParent.getLayoutImpl().updateChildrenOnRemoveChild( this, index );
            ...
            var parentNode = jsw.client.Client.isMshtml() ? this.getElement().parentElement : this.getElement().parentNode;
            if( parentNode ){
    ...
    addToCustomQueues : jsw.util.Functions.returnTrue,
    removeFromCustomQueues : jsw.util.Functions.returnTrue,
    _handleDisplayableCustom : jsw.util.Functions.returnTrue,
    ...
    _ieFixLayoutOnAppear : jsw.util.Variant.select( "qx.client", {
      "mshtml" : function() {
    ...
    _beforeDisappear : function() {
      ...
      if (jsw.qx.Class.isDefined("jsw.widgets.base.Button"))
      {
    ...
    destroy : function () {
    	var event = null;
        if( this.getParent() === null || !this.getParent()._isInGlobalDisposeQueue ) { // Se trata de un widget principal que va a ser eliminado. Notifica el propio widget y su padre
    	    event = new jsw.widgets.base.WidgetEvent(jsw.widgets.base.WidgetEvent.WidgetEventtype_DISPOSE, this, this.getParent());
    	    this._dispatchAsyncEvent(jsw.qx.Target.REMOVE_QUEUE, event);
    	    
    	    if (this.getParent() !== null) {
        	    event = new jsw.widgets.base.WidgetEvent(jsw.widgets.base.WidgetEvent.WidgetEventtype_CHILD_DISPOSE, this.getParent(), this);
        	    this.getParent()._dispatchAsyncEvent(jsw.qx.Target.REMOVE_QUEUE, event);
        	    try {
        	    	this.setParent( null );
        	    } catch (ex) {
        	    	// TODO: revisar por qué pasa esto.
        	    	console.error("Error al ejecutar 'this.setParent( null ); en bewf/widgets/base/Widget.js: 1886' ");
        	    }
    	    }
        } else {
        	event = new jsw.widgets.base.WidgetEvent(jsw.widgets.base.WidgetEvent.WidgetEventtype_PARENT_DISPOSE, this, this.getParent() );
    	    this._dispatchAsyncEvent(jsw.qx.Target.REMOVE_QUEUE, event);
        }
	    this._markInDispose();
    },
    ...
    addToDocument : function() {
      jsw.widgets.base.ClientDocument.getInstance().add(this);
    },
    ...
    _flushJobQueue : function(q) {
      ...
      var vParent = this.getParent();
      if (!vParent || jsw.util.Objects.isEmpty(vQueue)) {
        return;
      }
// TODO: Remove
/*
      var vLayoutImpl = this instanceof jsw.widgets.base.Parent ? this.getLayoutImpl() : null;
      if (vLayoutImpl) {
...
//        vParent.getLayoutImpl().updateSelfOnChildOuterHeightChange(this);
      ...
      if (this instanceof jsw.widgets.base.Parent && (vQueue.paddingLeft || vQueue.paddingRight || vQueue.paddingTop || vQueue.paddingBottom)) {
...
*/
    ...
    _isWidthEssential : jsw.util.Functions.returnTrue,
    _isHeightEssential : jsw.util.Functions.returnTrue,
...
/*
    _computeBoxWidth : function() {
      ...
      return Math.max(0, jsw.util.Numbers.limit(vLayoutImpl.computeChildBoxWidth(this), this.getMinWidthValue(), this.getMaxWidthValue()));
    },

    _computeBoxHeight : function() {
      var vLayoutImpl = this.getParent().getLayoutImpl();
      return Math.max(0, jsw.util.Numbers.limit(vLayoutImpl.computeChildBoxHeight(this), this.getMinHeightValue(), this.getMaxHeightValue()));
    },
...
*/
    ...
    _recomputeRangeX : jsw.util.Variant.select("qx.client", {
    ...
    _recomputeRangeY : jsw.util.Variant.select("qx.client", {
      "mshtml|newmshtml|opera|webkit" : function() {
    ...
    _recomputeStretchingX : jsw.util.Variant.select("qx.client", {
      "mshtml|newmshtml|opera|webkit" : function() {
    ...
    _recomputeStretchingY : jsw.util.Variant.select("qx.client", {
      "mshtml|newmshtml|opera|webkit" : function() {
...
*/
    ...
    _evalUnitsPixelPercentAutoFlex : function(value) {
      switch(value) {
        case "auto":
          return jsw.widgets.base.Widget.TYPE_AUTO;
        case Infinity:
        case -Infinity:
          return jsw.widgets.base.Widget.TYPE_NULL;
      }
      switch(typeof value) {
        case "number":
          return isNaN(value) ? jsw.widgets.base.Widget.TYPE_NULL : jsw.widgets.base.Widget.TYPE_PIXEL;

        case "string":
          return value.indexOf("%") != -1 ? jsw.widgets.base.Widget.TYPE_PERCENT : value.indexOf("*") != -1 ? jsw.widgets.base.Widget.TYPE_FLEX : jsw.widgets.base.Widget.TYPE_NULL;
      }
      return jsw.widgets.base.Widget.TYPE_NULL;
    },
    ...
    _evalUnitsPixelPercentAuto : function(value) {
      ...
      switch(typeof value) {
        case "number":
          return isNaN(value) ? jsw.widgets.base.Widget.TYPE_NULL : jsw.widgets.base.Widget.TYPE_PIXEL;
        case "string":
          return value.indexOf("%") != -1 ? jsw.widgets.base.Widget.TYPE_PERCENT : jsw.widgets.base.Widget.TYPE_NULL;
      }
      return jsw.widgets.base.Widget.TYPE_NULL;
    },
    ...
    _unitDetectionPixelPercentAutoFlex : function(name, value) {
      var r = jsw.widgets.base.Widget.layoutPropertyTypes[name];
      var s = r.dataType;
      var p = r.dataParsed;
      var v = r.dataValue;
      var s1 = r.typePixel;
     ...
     *  instance of jsw.widgets.base.ClientDocument).
     */
    ...
    getPreviousActiveSibling : function(vIgnoreClasses) {
      var vPrev = jsw.widgets.base.Widget.getActiveSiblingHelper(this, this.getParent(), -1, vIgnoreClasses, null);
      return vPrev ? vPrev : this.getParent().getLastActiveChild();
    },

    getNextActiveSibling : function(vIgnoreClasses) {
      var vNext = jsw.widgets.base.Widget.getActiveSiblingHelper(this, this.getParent(), 1, vIgnoreClasses, null);
    ...
    _styleFromMap : function( data ) {
      ...
      var styler = jsw.qx.Property.$$method.style;
      var unstyler = jsw.qx.Property.$$method.unstyle;
      var value;
    ...
    /*
      ...
      if (vAppearance) {
          var r = jsw.theme.AppearanceManager.getInstance().styleFrom(vAppearance, this.__states);
          if (r) {
    ...
    _resetAppearanceThemeWrapper : function(vNewAppearanceTheme, vOldAppearanceTheme) {
      var vAppearance = this.getAppearance();
      if (vAppearance) {
        var vAppearanceManager = jsw.theme.AppearanceManager.getInstance();
    ...
    _applyStateStyleFocus : jsw.util.Variant.select("qx.client", {
      "mshtml" : function(vStates) {},
      "default" : function(vStates) {
        if (vStates.focused) {
          if (!jsw.widgets.util.FocusHandler.mouseFocus && !this.getHideFocus()) {
    ...
    _ontabfocus : jsw.util.Functions.returnTrue,
    ...
    _applyHideFocus : jsw.util.Variant.select("qx.client", {
    ...
    _visualizeFocus : function() {
      var FocusHandler = jsw.widgets.util.FocusHandler;
    ...
    focus : function() {
      delete jsw.widgets.util.FocusHandler.mouseFocus;
    ...
    _applyCapture : function(value, old) {
      var vMgr = jsw.event.EventHandler;
      if (old) {
    ...
    _applyTabIndex : jsw.util.Variant.select("qx.client", {
      "mshtml" : function(value, old) {
        /*
    ...
    _applySelectable : jsw.util.Variant.select("qx.client", {
    ...
    _applyOpacity : function( value, old ) {
      jsw.html.Style.setOpacity( this, value );
    },
    ...
    __cursorMap : jsw.util.Variant.select("qx.client", {
      "mshtml" : {
        "default" : "default",
        "wait" : "wait",
        "crosshair" : "crosshair",
    ...
    _applyBackgroundImage : function(value, old) {
      var imageMgr = jsw.html.ImageManager.getInstance();
      if (old) {
        imageMgr.hide(old);
      }
      if (value) {
    ...
    _styleBackgroundImage : function( value ) {
      jsw.html.Style.setBackgroundImage( this, value );
    },

    _applyBackgroundRepeat : function( value, old ) {
      jsw.html.Style.setBackgroundRepeat( this, value );
    },

    _applyBackgroundPosition : function( value, old ) {
      jsw.html.Style.setBackgroundPosition( this, value );
    },
    ...
    _applyOverflow : jsw.util.Variant.select("qx.client", {
      "default" : function(value, old) {
      ...
      "opera" : function(value, old) {
        ...
        var pv = value;
        var pn = "overflow";
        if( jsw.client.Client.getVersion() < 9.8 ) {
          switch( pv ) {
            case "scrollX":
            case "scrollY":
              pv = "scroll";
    ...
    _applyBackgroundGradient : function( value, oldValue ) {
      //jsw.html.Style.setBackgroundGradient( this, value );
    },

    _applyShadow : function( value, oldValue ) {
      //jsw.html.Style.setBoxShadow( this, value );
    },

    _applyTextShadow : function( value, oldValue ) {
      //jsw.html.Style.setTextShadow( this, value );
    },

    _styleBackgroundColor : function( value ) {
      //jsw.html.Style.setBackgroundColor( this, value );
    },
    ...
    renderBorder : function( changes ) {
      ...
      if( value ) {
        ...
        jsw.html.Border.resetWidget( this );
      }
    ...
    getOffsetLeft : function() {
      this._visualPropertyCheck();
      return jsw.html.Offset.getLeft(this.getElement());
    },

    getOffsetTop : function() {
      this._visualPropertyCheck();
      return jsw.html.Offset.getTop(this.getElement());
    },
    ...
    scrollIntoViewX : function(alignLeft) {
      ...
      return jsw.html.ScrollIntoView.scrollX(this.getElement(), alignLeft);
    },
    ...
    scrollIntoViewY : function(alignTop) {
      ...
      return jsw.html.ScrollIntoView.scrollY(this.getElement(), alignTop);
    },
    ...
    ////////////////////////////////
    // CHANGE PROPERTY EVENT WRAPPER
    _dispatchAsyncChangePropertyEvent : function(property, oldValue, newValue) {
  		var evt = new jsw.widgets.base.WidgetEvent( jsw.widgets.base.WidgetEvent.WidgetEventType_PROPERTY, this );
  		evt.property = property;
  		evt.oldValue = oldValue;
  		evt.newValue = newValue;
  		this._dispatchAsyncEvent( jsw.qx.Target.SET_QUEUE, evt);
    }

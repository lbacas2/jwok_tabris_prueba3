...
jsw.qx.Class.define( "renderer.tabris.SideBarModuleRenderer", {
	...
	members : {
		...
		_updateItem : function() {
			if ( this.getParent() !== null && this.__sideBarRenderer !== null ) {
				...
				this.__itemData = { 'id': this.getJSWWidget().getInternalId(), 'text': this.getJSWWidget().getText(), 'image': this.getJSWWidget().getImage(), 'leaf' : false };
...
renderer.base.RendererHandlerRegistry.getInstance().add("jsw.widgets.sidebar.SideBarModule", {
	create : function() {
		return new renderer.tabris.SideBarModuleRenderer();
	}
});
